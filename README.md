...
[![LLM Gemini 3.5 Flash](https://img.shields.io/badge/LLM-Gemini_3.5_Flash-4285F4.svg?logo=google&logoColor=white)](https://ai.google.dev/)
[![Embeddings](https://img.shields.io/badge/Embeddings-Hugging_Face_Serverless-yellow.svg)](https://huggingface.co/)
[![Vector Store FAISS](https://img.shields.io/badge/Vector_Store-FAISS-00599C.svg)](https://github.com/facebookresearch/faiss)
[![Lexical Rank BM25](https://img.shields.io/badge/Lexical-Rank_BM25-brightgreen.svg)](https://pypi.org/project/rank-bm25/)
[![Deploy Render.com](https://img.shields.io/badge/Deploy-Render.com-black.svg?logo=render&logoColor=white)](https://render.com)
...
Pakistan's corporate and fiscal environment is governed by complex, rapidly shifting statutory frameworks: the **Income Tax Ordinance 2001**, annual **Finance Acts**, **Sales Tax Act 1990**, and **SECP Companies Regulations**. Corporate counsels, tax practitioners, CFOs, and SMEs encounter three major bottlenecks:

1. **The Math-Legal Disconnect**: Legal questions are deeply intertwined with progressive tax math (e.g., Section 4C Super Tax on high-earning persons, progressive salaried slabs, and high-income surcharges).
2. **The Hallucination Danger**: Generic LLMs (ChatGPT-4o, Claude 3.5) hallucinate Pakistani tax brackets, invent non-existent statutory sections, or cite outdated 2021 rates.
3. **Statutory Limitation Defaults**: Over 70% of taxpayers forfeit appeal rights before the Commissioner (Appeals) simply because they miss the strict 30-day statutory limitation window under Section 127.

**Pakistan BusinessTax HUB** bridges this gap through an **Agentic Hybrid RAG & Multi-Agent Architecture**:
...
* **Hybrid Semantic & Lexical Retrieval (Hugging Face + FAISS + BM25)**: Dense vector embeddings powered by Hugging Face Serverless API (`sentence-transformers/all-MiniLM-L6-v2`) and FAISS Inner Product search fused with Rank-BM25 via Reciprocal Rank Fusion (RRF).
* **Instant Answer Streaming with Sources Below**: Answers stream progressively in real time, followed by verified statutory citation chips at the bottom.
* **Autonomous Compliance Actions**: Calculates statutory limitation periods with 1-Click Google Calendar synchronization and official FBR IRIS / SECP eZfile portal guidance.
...
graph TD
    ...
    subgraph Workers [3. Specialized Worker Agents]
        ...
        LegalRetriever["📚 Agent B: Hybrid Legal Retriever<br>• Hugging Face Serverless Embeddings<br>• Dense Semantic Search (FAISS)<br>• Sparse Lexical Search (Rank-BM25)<br>• Reciprocal Rank Fusion (RRF)<br>• Urdu/Roman Urdu Query Rewriter"]
    end
    ...
    subgraph Actions [5. Action Agents]
        CalAgent["📅 Compliance Calendar Agent<br>• Calculates Section 127 30-day deadline<br>• 1-Click Google Calendar Sync (.ics)"]
        PortalAgent["🏛️ Official Portal Link Agent<br>• Verified FBR IRIS & SECP eZfile Links<br>• 4-Step Statutory Filing Checklist"]
    end

    subgraph Frontend [6. User Interfaces]
        ChatUI["ChatGPT-Style Legal Studio<br>• Dark Obsidian & Light Mode<br>• Answer-First Streaming Engine<br>• Verified Statutory Sources Chips<br>• Speech STT & Read-Aloud TTS"]
